subroutine do_whiten(sachead,fre,sig,seisout)
use sacio
implicit none
type(sac_head) :: sachead
integer,parameter:: nmax=4000000
integer nerr,i,nk,npts,npow,ns
real*4 fre(4),dt,amax
real*8 dom,f
real*4 sig(nmax,3),seisout(nmax,3)
...
call smoothf(dom,nk,20,fre,sa,ss1)
call smoothf(dom,nk,20,fre,sb,ss2)
call smoothf(dom,nk,20,fre,sc,ss3)
...
enddo
! taper the spectrum
write(*,*)"Taper the spectrum!",dom,nk,npow
...
seisout(1:npts,1) = real(s1(1:npts))
seisout(1:npts,2) = real(s2(1:npts))
seisout(1:npts,3) = real(s3(1:npts))
end subroutine
